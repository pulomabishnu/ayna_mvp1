...
function SpecRow({ label, value }) {
  return (
    <div style={{ display: 'flex', gap: 16, padding: '15px 0', borderBottom: '1px solid var(--ayna-border)' }}>
      <div style={{ fontSize: 'calc(13.5px * var(--ayna-text-scale, 1))', color: 'var(--ayna-text-faint)', flex: 'none', width: 82 }}>{label}</div>
      <div style={{ flex: 1, fontSize: 'calc(13.5px * var(--ayna-text-scale, 1))', lineHeight: 1.5, color: 'var(--ayna-text)', textAlign: 'right' }}>{value}</div>
    </div>
  );
}
...
function EvidenceRow({ label, value }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, padding: '12px 0', borderTop: '1px solid var(--ayna-chip-bg)' }}>
      <div style={{ fontSize: 'calc(13.5px * var(--ayna-text-scale, 1))', color: 'var(--ayna-text)' }}>{label}</div>
      <div style={{ fontSize: 'calc(13.5px * var(--ayna-text-scale, 1))', color: 'var(--ayna-text-muted)', textDecoration: 'underline', textDecorationColor: 'var(--ayna-border)' }}>{value}</div>
    </div>
  );
}
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', background: 'var(--ayna-bg)', color: 'var(--ayna-text)', animation: 'ay-page .25s ease-out' }}>
      ...
      <div style={{ padding: '22px 22px 34px' }}>
        {categoryLabel && (
          <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 'calc(10px * var(--ayna-text-scale, 1))', letterSpacing: '1.4px', textTransform: 'uppercase', color: 'var(--ayna-accent-dark)' }}>
            {categoryLabel}
          </div>
        )}
        <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 'calc(33px * var(--ayna-text-scale, 1))', lineHeight: 1.12, margin: '9px 0 10px', color: 'var(--ayna-heading)' }}>{name}</div>
        {price && <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 'calc(26px * var(--ayna-text-scale, 1))', color: 'var(--ayna-heading)', marginBottom: 12 }}>{price}</div>}
        {summary && <div style={{ fontSize: 'calc(14.5px * var(--ayna-text-scale, 1))', lineHeight: 1.55, color: 'var(--ayna-text-muted)' }}>{summary}</div>}
        ...
        {activeTab === 'summary' ? (
          ...
          <>
            {whyMatched && (
              ...
              >
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 'calc(9.5px * var(--ayna-text-scale, 1))', letterSpacing: '1.3px', textTransform: 'uppercase', opacity: 0.62 }}>
                  Why you're seeing this
                </div>
                <div style={{ fontSize: 'calc(14.5px * var(--ayna-text-scale, 1))', lineHeight: 1.5, marginTop: 9 }}>{whyMatched}</div>
              </div>
            )}
            ...
            {evidenceRows.length > 0 && (
              <div style={{ background: 'var(--ayna-surface)', border: '1px solid var(--ayna-border)', borderRadius: 20, padding: '17px 18px' }}>
                <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 'calc(9.5px * var(--ayna-text-scale, 1))', letterSpacing: '1.3px', textTransform: 'uppercase', color: 'var(--ayna-text-faint)', marginBottom: 4 }}>
                  Evidence
                </div>
        ...
        {reads.length > 0 && (
          <>
            <div style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 600, fontSize: 'calc(14px * var(--ayna-text-scale, 1))', margin: '22px 0 10px' }}>Reads</div>
            {reads.map((r) => (
              <div
                key={r.id || r.title}
                onClick={r.onClick}
