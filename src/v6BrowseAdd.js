...
function buildHelper(input, source) {
  ...
  helper.querySelector('button').addEventListener('click', () => {
    if (clean(input.value).length < 2) {
      helper.querySelector('.v6-add-product-status').textContent = 'Type the product name in the search field, then tap + Add.';
      input.focus();
      return;
    }
    submitProduct(input.value, source, helper);
  });
  return helper;
}
...
function syncInput(input, source) {
  ...
  label.textContent = query ? `Can’t find “${query}”?` : 'Missing a product?';
  buttonText.textContent = query ? `Add “${query}”` : 'Add product';
