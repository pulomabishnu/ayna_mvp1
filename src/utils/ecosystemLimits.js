...
export function isAutoGeneratedEcosystemProduct(product) {
  return !!product
    && (product.llmGenerated === true || product.intakeGenerated === true)
    && product._userSwapped !== true;
}
...
/**
 * Keep at most maxPerCategory AUTO-GENERATED products in each broad ecosystem
 * category. Manual additions and user swaps are always retained. Callers can
 * additionally protect generated products that the user tracked or saved.
 *
 * Selection prefers explicitly-protected products, then higher stored match
 * scores, then the original recommendation order. Protected products do not
 * consume the generated allowance. The returned list is restored to original
 * order for stable UI layout.
 *
...
export function limitEcosystemProductsByCategory(
  ...
  { priorityIds = new Set(), protectedIds = priorityIds, capAllUnprotected = false } = {}
) {
  ...
  indexed.sort((a, b) => {
    const aPriority = protectedSet.has(a.product.id) ? 1 : 0;
    const bPriority = protectedSet.has(b.product.id) ? 1 : 0;
  ...
  for (const { product } of indexed) {
    if ((!capAllUnprotected && !isAutoGeneratedEcosystemProduct(product)) || protectedSet.has(product.id)) {
