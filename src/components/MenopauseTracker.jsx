...
const SYMPTOMS = ['Hot Flashes', 'Night Sweats', 'Brain Fog', 'Joint Pain', 'Sleep Disruption', 'Mood Swings', 'Vaginal Dryness', 'Fatigue'];
const INTENSITY_LEVELS = ['None', 'Mild', 'Moderate', 'Severe'];

export default function MenopauseTracker({ menopauseData, setMenopauseData, myProducts, trackedProducts, onToggleMyProduct, onToggleTrackedProduct, omittedProducts, toggleOmitProduct, onOpenProduct }) {
    const [currentMonth, setCurrentMonth] = useState(new Date());
    const [selectedDate, setSelectedDate] = useState(new Date().toISOString().split('T')[0]);
    const [activeTab, setActiveTab] = useState('log'); // 'log', 'recommendations', 'insights'
    ...
    useEffect(() => {
        const existingEntry = menopauseData.find(e => e.date === selectedDate);
        if (existingEntry) {
    ...
    const handleSave = () => {
        ...
        setMenopauseData(prev => {
            ...
            if (intensity || entry.symptoms.length > 0 || notes) {
    ...
    // ----- Recommendation Logic based on current logged symptoms/intensity -----
    const recommendationGroups = useMemo(() => {
        if (!intensity && selectedSymptoms.size === 0) return [];

        const groups = [];

        if (intensity === 'Severe' || intensity === 'Moderate') {
            const painMatches = ALL_PRODUCTS.filter(p => p.tags?.includes('pain') || p.tags?.includes('discomfort') || p.name?.includes('Relief')).map(p => ({ ...p, recommendedFor: 'Overall Discomfort' }));
            if (painMatches.length > 0) groups.push({ label: 'Severity Management', products: painMatches.slice(0, 3) });
        }

        const symptomConfigs = [
            { id: 'Hot Flashes', tags: ['menopause', 'cooling'], names: ['Cool', 'Menopause'], label: 'Hot Flashes' },
            { id: 'Night Sweats', tags: ['sleep', 'cooling'], names: ['Sleep', 'Cool'], label: 'Night Sweats' },
            { id: 'Brain Fog', tags: ['focus', 'brain', 'cognitive'], names: ['Mane', 'Focus', 'Omega'], label: 'Brain Fog' },
            { id: 'Joint Pain', tags: ['pain', 'joint', 'anti-inflammatory'], names: ['Turmeric', 'Joint'], label: 'Joint Pain' },
            { id: 'Sleep Disruption', tags: ['sleep', 'insomnia'], names: ['Magnesium', 'Sleep'], label: 'Sleep Disruption' },
            { id: 'Mood Swings', category: 'mental-health', tags: ['mood'], label: 'Mood Swings' },
            { id: 'Vaginal Dryness', tags: ['intimate-care', 'dryness', 'lubricant', 'hydration'], category: 'intimate-care', label: 'Vaginal Dryness' },
        ...
        // Add a general menopause tracker group if we lack matched groups
        if (groups.length === 0 && selectedSymptoms.size > 0) {
            let menopauseMatches = ALL_PRODUCTS.filter(p => p.category === 'menopause' || p.tags?.includes('menopause')).map(p => ({ ...p, recommendedFor: 'Menopause Support' }));
            if (menopauseMatches.length > 0) groups.push({ label: 'General Menopause Support', products: menopauseMatches.slice(0, 3) });
        }

        return groups;
    }, [intensity, selectedSymptoms]);

    // ----- Insights Logic -----
    const insights = useMemo(() => {
        if (!menopauseData || menopauseData.length === 0) return null;

        let totalSymptoms = {};
        let activeDays = 0;
        let intensityCount = { 'None': 0, 'Mild': 0, 'Moderate': 0, 'Severe': 0 };

        menopauseData.forEach(entry => {
            if (entry.intensity && entry.intensity !== 'None') {
                activeDays++;
                intensityCount[entry.intensity] = (intensityCount[entry.intensity] || 0) + 1;
            }
            if (entry.symptoms) {
        ...
        const sortedIntensity = Object.entries(intensityCount).filter(f => f[1] > 0).sort((a, b) => b[1] - a[1]);
        const commonIntensity = sortedIntensity.length > 0 ? sortedIntensity[0][0] : 'None logged';

        return {
            totalEntries: menopauseData.length,
            activeDays,
            topSymptom,
            commonIntensity
        };
    ...
    return (
        <div className="container animate-fade-in-up" style={{ padding: 'var(--spacing-lg) var(--spacing-md)', maxWidth: '1100px' }}>
            <div style={{ textAlign: 'center', marginBottom: 'var(--spacing-lg)' }}>
                ...
                <h2 style={{ fontSize: '2.5rem', marginBottom: '1rem', color: 'var(--color-text-main)' }}>Menopause Tracker</h2>
                <p style={{ color: 'var(--color-text-muted)', fontSize: '1.05rem', maxWidth: '600px', margin: '0 auto' }}>
                    Track your daily symptoms and identify patterns to help manage your transition smoothly.
                </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.2fr) minmax(0, 1fr)', gap: '3rem', alignItems: 'start' }}>
                ...
                <div className="card" style={{ padding: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '0.5rem' }}>
                        {calendarDays.map((dateStr, idx) => {
                            ...
                            const entry = menopauseData.find(e => e.date === dateStr);
                            const hasIntensity = !!(entry && entry.intensity && entry.intensity !== 'None');
                            ...
                            return (
                                <div
                                    ...
                                    style={{
                                        ...
                                        fontSize: '0.95rem',
                                        transition: 'all 0.2s',
                                        background: isSelected ? 'var(--color-primary)' : hasIntensity ? 'var(--color-secondary)' : 'transparent',
                                        color: isSelected ? 'white' : hasIntensity ? 'var(--color-primary-hover)' : 'var(--color-text-main)',
                                ...
                                >
                                    ...
                                    <div style={{ display: 'flex', gap: '2px', position: 'absolute', bottom: '15%' }}>
                                        {hasIntensity && !isSelected && <span style={{ width: '4px', height: '4px', borderRadius: '50%', background: 'var(--color-primary-hover)' }}></span>}
                ...
                {/* Right Panel: Tabs for Logger, Recommendations, Insights */}
                    ...
                    <div style={{ padding: '2rem' }}>
                        {activeTab === 'log' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'fadeInUp 0.3s ease' }}>
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '600', color: 'var(--color-text-main)', fontSize: '1rem' }}>Overall Symptom Intensity</label>
                                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                        {INTENSITY_LEVELS.map(f => (
                                            <button
                                                key={f}
                                                onClick={() => setIntensity(intensity === f ? '' : f)}
                                                className={`btn ${intensity === f ? 'btn-primary' : 'btn-outline'}`}
                                                style={{
                                                    fontSize: '0.8rem',
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '600', color: 'var(--color-text-main)', fontSize: '1rem' }}>Symptoms Experienced</label>
                                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                                        {SYMPTOMS.map(s => {
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'fadeInUp 0.3s ease' }}>
                                ...
                                {recommendationGroups.length === 0 ? (
                                    <div style={{ padding: '2rem 1rem', textAlign: 'center', background: 'var(--color-bg)', borderRadius: 'var(--radius-md)', border: '1px dashed var(--color-border)' }}>
                                        ...
                                        <p style={{ color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>Log symptoms or intensity in your Daily Log to see personalized action items.</p>
                                    </div>
                                ) : (
                        ...
                        {activeTab === 'insights' && (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'fadeInUp 0.3s ease' }}>
                                <div>
                                    <h3 style={{ fontSize: '1.3rem', color: 'var(--color-text-main)', marginBottom: '0.5rem' }}>Your Menopause Insights</h3>
                                    <p style={{ color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>
                                ...
                                {insights ? (
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                        <div style={{ background: 'var(--color-surface-soft)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', textAlign: 'center' }}>
                                            ...
                                            <div style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)', fontWeight: '500', textTransform: 'uppercase' }}>Most Common Intensity</div>
                                            <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--color-primary)', marginTop: '0.2rem' }}>{insights.commonIntensity}</div>
                                        </div>
                                        <div style={{ background: 'var(--color-surface-soft)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', textAlign: 'center' }}>
                                            <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>🤒</div>
                                        ...
                                        <div style={{ background: 'var(--color-surface-soft)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', textAlign: 'center' }}>
                                            <div style={{ fontSize: '2rem', marginBottom: '0.5rem' }}>📉</div>
                                            <div style={{ fontSize: '0.9rem', color: 'var(--color-text-muted)', fontWeight: '500', textTransform: 'uppercase' }}>Active Symptom Days</div>
                                            <div style={{ fontSize: '1.2rem', fontWeight: 'bold', color: 'var(--color-primary)', marginTop: '0.2rem' }}>{insights.activeDays} Days</div>
                                        </div>
                                    </div>
                                ) : (
                                    <div style={{ padding: '2rem 1rem', textAlign: 'center', background: 'var(--color-bg)', borderRadius: 'var(--radius-md)', border: '1px dashed var(--color-border)' }}>
                                        <div style={{ fontSize: '2rem', marginBottom: '1rem', opacity: '0.5' }}>📊</div>
                                        <p style={{ color: 'var(--color-text-muted)', fontSize: '0.95rem' }}>Log more data to see your menopause insights over time.</p>
                                    </div>
                                )}
                            </div>
                        )}
